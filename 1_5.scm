...
Normal-order evaluation will not evaluate the operands until their values are needed. Since the predicate expression
is evaluated first, and the result of the predicate is true, (p) is never evaluated. Thus, the result is the consequent, which is 0. 

With applicative order evaluation, the operator and operands are evaluated first. When y is evaluated, we get (p), 
which then calls itself in an infinite loop. The procedure will never finish.
